...
def printPayload(l):
    ...
    # ペイロードをそのまま出力する
    payload = l[2:]
    print("  payload = %d byte" % len(payload))

    print('hex: ' + ' '.join('%02x' % b for b in payload))
    print('asc: ' + repr(payload.decode('ascii')))
...
def serialize_uart_command(txt, target=0x78):
    """
    Convert UART command ("p", "e1t100" etc.) to
    TWELITE standard app modbus command.

    target: integer ID (<256), default(0x78) is broadcast
    returns: str
    """
    ...
    # Convert to ASCII modbus.
    return ':%s\r\n' % codecs.encode(binary_command, "hex_codec").decode('ascii')
    ...
    while True:
        try:
            ...
            if len(l) > 0:
                ...
                if l[0] == ':':
                    command = serialize_uart_command(l[1:].strip())
                    print ("--> "+ command)
